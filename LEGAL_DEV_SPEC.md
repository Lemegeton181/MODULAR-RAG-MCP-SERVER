...
| Phase J - 本地模型接入 + 离线自检 + 交付文档 | [~] | - | 新增 src/legal/local_llm.py（LlamaCppClient + LEGAL_LLM_MODEL_PATH 解析 + 缺依赖/缺模型清晰报错，llama_cpp 延迟导入）；legal_answer CLI 加 --llm/--model-path；新增 scripts/legal_self_check.py（PASS/WARN/FAIL，LLM 缺失仅 WARN）；新增 models/{README,llm,embedding,ocr} 目录与 .gitkeep；新增 start_legal_demo.bat 与 README_LEGAL.md（一句话/链路/安装/运行/评测/离线边界/TODO/1 分钟讲法）；新增 tests/legal/test_legal_local_llm.py、tests/legal/test_legal_self_check.py。等本地 pytest 全绿后置 [x] |
